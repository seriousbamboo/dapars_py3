...
def De_Novo_3UTR_Identification_Loading_Target_Wig_for_TCGA_Multiple_Samples_Main(argv=None):
    ...
    All_Sample_files.extend(Group2_Tophat_aligned_file) # ['Condition_A_chrX.wig', 'Condition_B_chrX.wig']
    ...
    All_sample_coverage_weights = All_samples_sequencing_depths/np.mean(All_samples_sequencing_depths) #vector
    ...
    for curr_3UTR_id in UTR_events_dict:
        ...
        # Obtain the structure of current 3'UTR
        # Example:
        # UTR_events_dict = {'NM_001037968|ALAS2\r|chrX|-':
        # ['chrX', 55035547, 55035775, '-', 'chrX:55035488-55035776']}
        ...
        # must include in the target:
        if curr_3UTR_id in All_samples_Target_3UTR_coverages:
        
            # Extract the id of UTR event
            # example: All_samples_Target_3UTR_coverages = {'NM_001037968|ALAS2\r|chrX|-':
            #          [[[1, 2, 1, 2, 1, 0],
            #            [55035547, 55035558, 55035582, 55035650, 55035659, 55035751, 55035775]],
            #           [[0],
            #            [55035547, 55035775]]]}     =>
            #
            #          curr_3UTR_curr_sample_bp_coverage = [[sample1_cov_1bp],[sample2_cov_1bp]]  =>
            #
            #          get result from De_Novo_3UTR_Coverage_estimation_Genome_for_TCGA_multiple_samples => write result
            ...
            # curr_3UTR_coverage_wig[0] =
            # [[1, 2, 1, 2, 1, 0],
            #  [55035547, 55035558, 55035582, 55035650, 55035659, 55035751, 55035775]]
            # curr_3UTR_coverage_wig[1] =
            # [[0],
            #  [55035547, 55035775]]
            ...
            for curr_sample_curr_3UTR_coverage_wig in curr_3UTR_coverage_wig:
                ...
                # iteration 1:
                # curr_3UTR_curr_sample_bp_coverage[0] =
                # [1, 2, 1, 2, 1, 0]
                # curr_3UTR_curr_sample_bp_coverage[1] =
                # [55035547, 55035558, 55035582, 55035650, 55035659, 55035751, 55035775]
                ...
                # Append the vector
                # curr_3UTR_curr_sample_bp_coverage = [[sample1],[sample2]]
            ...
            # UTR_abundances =
            # [[sample_1_opt_point_long_abun,  sample_2_opt_point_long_abun, ...],
            #  [sample_1_opt_point_short_abun, sample_2_opt_point_short_abun, ...]]
...
def De_Novo_3UTR_Coverage_estimation_Genome_for_TCGA_multiple_samples(All_Samples_curr_3UTR_coverages, UTR_start, UTR_end,curr_strand,weight_for_second_coverage):
    ...
    # Region_Coverages = [[sample 1 coverage],[sample 2 coverage]]
    
    
    # For this UTR, some requirement need to be validate to jump into the iteration and calculation:
    # (basically it delect short UTR and delect low coverage UTR)
    
    if sum(np.array(Region_first_100_coverage_all_samples) >= coverage_threshold) >= num_samples and UTR_end - UTR_start >= 150:
        
        # Identify search region and the start/end point:
        ...
        # Iteration point by point, calculate relevant stats by sample
        ...
        for curr_point in range(search_region_start, search_region_end+1):
            ...
            # Split by sample and do iteration:
            ...
            ################# After this inner loop ###################################
            #
            # All_samples_result = [ [sample_1_mse, sample_2_mse, ...],
            #                        [sample_1_long_abun,  sample_2_long_abun, ...],
            #                        [sample_1_short_abun, sample_2_short_abun,...] ]
            #
            ################# Next loop will reset  ###################################
        ...
        # Final result:
        #
        # Mean_squared_error_list = [mse1_all_sample_mean, mse2_all_sample_mean, ...]       1D
        # Estimated_3UTR_abundance_list = [ [[sample_1_point_1_long_abun,  sample_2_point_1_long_abun, ...],
        #                                    [sample_1_point_1_short_abun,  sample_2_point_1_short_abun, ...]],
        #                                   [[point_2_all_sample_long_abun], [point_2_all_sample_short_abun]],
        #                                  ...]
        #
        #
        ########################################################################################################
        
        # return the identified minimum value:
...
def Estimation_abundance(Region_Coverage, break_point):
    ...
    # coverage = [point1,point2,.....]
    # break_point = number (as we will iteration all point 1 by 1)
...
def Load_Target_Wig_files(All_Wig_files, UTR_Annotation_file):
    ...
    for line in open(UTR_Annotation_file,'r'):
        ...
        # sample output after ...
        # line 1:
        # ('chr14', '50792327', '50792946', '+', 'chr14:50792327-50792946', 124)
        ...
        # sample output after ...
        # item 0 in dictionary:
        # {'NM_005644|TAF12\r|chr1|-': ['chr1', 28929705, 28930085, '-', 'chr1:28929609-28930086']
        # refer to this line of raw data:
        # chr1    28929609    28930086    NM_005644|TAF12   |chr1|-    0    -
    ...
    for curr_wig_file in All_Wig_files:
        ...
        for line in open(curr_wig_file,'r'):
            if '#' not in line and line[0:3] == 'chr':
                ...
                # Dictionary is based on chormosom. Firstly add a header {chrom_name : [[0],[0]] }.
                # Then we will extend each element in the value vector.
                # Below is for creating the header:
                ...
                # The fisrt element of the vector refer to the interval.
                # The second element of the vector refer to reads in interval.
        ...
        #chrX        0             193010        0
        #chrX        193010        193020        1
        #chrX        193020        193031        2
        #chrX        193031        193032        3
        #chrX        193032        193034        4
        #chrX        193034        193037        5
        #chrX        193037        193039        6

        # output in last setp:
        #{'chrX':
        #[  [0, 193010, 193020, 193031, 193032, 193034, 193037, 193039],
        #[0,   0,      1,      2,      3,      4,      5,       6,      0]]}

        # note that the second vector is longer than the first by 1 for control
        
        
        
        # Create new dictionary All_samples_extracted_3UTR_coverage_dict:
        
        for curr_3UTR_event_id in UTR_events_dict:
            curr_3UTR_structure = UTR_events_dict[curr_3UTR_event_id]  # extract value
            curr_chr = curr_3UTR_structure[0]                          # extract chomosome
            ...
            if curr_chr in curr_sample_All_chroms_coverage_dict:
                ...
                region_start = curr_3UTR_structure[1]  # adjusted utr start
                region_end = curr_3UTR_structure[2]    # adjusted utr end
                ...
                # Add key-value to the new dictionary
